...
/**
 *
 * @author stefan.busnita Used for password and username encryption/decryption
 */
@Component
public class Crypto {
    ...
    public String encrypt(String plaintext) throws InvalidKeySpecException {
        try {
            SecretKey key = generateKey(cryptoProps.getSalt(), cryptoProps.getPassphrase());
            byte[] encrypted = doFinal(Cipher.ENCRYPT_MODE, key, cryptoProps.getIv(), plaintext.getBytes("UTF-8"));
    ...
    private SecretKey generateKey(String salt, String passphrase) throws InvalidKeySpecException {
        try {
            ...
            KeySpec spec = new PBEKeySpec(passphrase.toCharArray(), hex(salt), cryptoProps.getIterationCount(), cryptoProps.getKeySize());
